...
{
	int j,n,nr,Nchem,nRt,Rt,rev,J,Nn,Nvib,k,Ki;
...
void chem_read_react(int Nchem)//считывание процессов
{
...
void chem_const(double Tel,double Tch,int nt,int dot)// расчёт констант скоростей реакций
{
	int j,n,Sum;
	double Kf;
	...
	for(j=0;j<Nchem;j++)
	{
        ...
        if(!strcmp(Rtype[j],"VV"))//запись констант реакций в виде - k*(T)^n*exp(-Ea/T^0.33), k-[см^3/с], Ea-[K]
        {
            ...
            Nvv = chem_VV_VT_const(j,"VV",kv10,Tch);
        ...
        if(!strcmp(Rtype[j],"VT"))//запись констант реакций в виде - k*(T)^n*exp(-Ea/T^0.33), k-[см^3/с], Ea-[K]
        {
            ...
            Nvt = chem_VV_VT_const(j,"VT",kv10,Tch);
...
int chem_VV_VT_const(int j,char vin[],double kv10,double Tch)// расчёт констант скоростей колебательных реакций
{
...
void chem_runge_kutta4(double Ni[],int nt,int dot)//расчёт по времени методом Рунге-Кутта 4 порядка
{
	int i,n,j,l;
	...
	if(dot==Ndots)
		chem_spec_contrib(nt);
	//******************************************************************************

}
void chem_half_implicit(double Ni[],int nt,int dot)//расчёт полунеявный по времени методом 1 порядка
{
	double Y[N],Y0[N];
	double Si[N],Li[N],Ki[N],Res[N];
	double Ysum,Q,A,B,C,Y_M;
	...
	*/
	...
	for(m=0;m<N;m++)
	{
		//Si[m] += RCvDf[m]; //неверно, получается,что входит поправка от уже расчитанных потоков в тот же момент.
		Q = dt*Li[m];
		if(Q>=0.1)
		{
			A = 1.0+Q;
			...
			Y[m] = Y0[m]+dt*(Si[m]-Li[m]*Y0[m]-Ki[m]*Y0[m]*Y0[m]);

		Ni[m] = Y[m];
	}
	Nel = Ni[0];//концентрация электронов
...
void chem_spec_contrib(int nt)//вывод скоростей реакций для выбранных компонент
{
	FILE *rrp;
	FILE *rcp;
