                        ...
                        if(m==7)
                        {
                            for(j=1;j<=Jmax;j++)
                            {
                                ...
                                */
                                ...
                                double E_kT = kb*Tgas;//E_kT=kT(Волошин)
...
void EEDF_const_calc()//Kel(EEDF)-calculation
{
...
void EEDF_print(int nt,double Norm)//запись EEDF в файл
{
	int k;
    ...
    fprintf(log,"t=%.2e[s]\t %.2lf\t",nt*dt,Te);
	for(k=0; k<NEmax; k++)
	{
